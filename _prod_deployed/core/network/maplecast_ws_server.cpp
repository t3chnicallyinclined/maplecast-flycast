...
static void checkMatchEnd()
{
	...
	if (gs.in_match) {
		...
		if (!_matchActive) {
			...
			// Delay MCSV build: round intro triggers GD-ROM reads for ~5s.
			// A savestate captured mid-read causes "Failed to locate bootfile"
			// on no-ROM replica clients. 300 frames = 5s at 60 fps.
		...
		// Relay case only: onOpen covers direct connections. 60s interval avoids sending
		// ~5MB savestate to all viewers every 5s.
		{
