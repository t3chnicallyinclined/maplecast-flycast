// shot_stage_headless.mjs — PROVE the Phase-3 STAGE pass actually renders.
//
// Loads the LIVE camera matrices + stage_id out of a real _satlive.mcrr capture, drives
// the UNMODIFIED StageClient (web/webgpu/stage-client.mjs) with them, builds the parsed
// geometry, and renders it through the SAME PVR2Renderer the live replay.html uses — to an
// offscreen WebGPU target, read back to a PNG. Non-black pixel count proves the stage draws.
//
//   node shot_stage_headless.mjs [file.mcrr] [out.png]
...
const RECF = process.argv[2] || join(HERE, '..', '..', '_satlive.mcrr');
const OUT  = process.argv[3] || join(HERE, 'PNG_stage_headless.png');
const STAGE_DIR = join(HERE, '..', '..', 'web', 'test-atlas', 'stages');
const W = 640, H = 480;

// ---- shim fetch (local file) + createImageBitmap (pngjs) for StageClient ----
globalThis.fetch = async (url) => {
  let p = String(url);
  if (p.startsWith('file://')) p = new URL(p).pathname.replace(/^\/([A-Za-z]:)/, '$1');
...
// ---- read live matrices + stage_id from mcrr ----
...
const stageId = ram[G(0x8C289638)];
const M1 = mat16(0x8C2D6B18), M2 = mat16(0x8C2D6AD8);
console.log(`stage_id=${stageId} (STG${stageId.toString(16).toUpperCase().padStart(2, '0')}) | M2[15]=${M2[15].toFixed(2)} M1[0]=${M1[0].toFixed(2)}`);

// ---- device + renderer (offscreen) ----
...
// monkeypatch copyExternalImageToTexture -> writeTexture from our pngjs _rgba bitmap
const origCopyExt = device.queue.copyExternalImageToTexture?.bind(device.queue);
...
// ---- drive StageClient with the live camera ----
const stage = new StageClient(pathToFileURL(STAGE_DIR).href);
stage.attachDevice(device);
stage.setState(stageId, 0);
// _ensureLoaded is async; wait for it
for (let i = 0; i < 200 && stage.stageId !== stageId; i++) await new Promise(r => setTimeout(r, 25));
if (stage.stageId !== stageId) throw new Error('stage failed to load');
stage.setCamera(M1, M2);
const p = stage.parsed, tm = stage.texMgr;
console.log(`[stage] loaded STG: parsed.vertexCount=${p ? p.vertexCount : 'NULL'} opaque=${p ? p.opaque.length : '-'} trans=${p ? p.translucent.length : '-'} texMgr=${tm ? 'yes' : 'NULL'}`);

// ---- render target ----
function makeRT() {
  const color = device.createTexture({ size: [W, H], format: R.fmt, usage: GPUTextureUsage.RENDER_ATTACHMENT | GPUTextureUsage.COPY_SRC | GPUTextureUsage.TEXTURE_BINDING });
...
// EXACTLY the renderStagePass() call from replay.html:
R.renderFrame(p, tm, STAGE_PVRSNAP, null,
  { singlePass: true, noSort: true, drawOpaque: true, drawPunch: false, drawTrans: true }, rt);
...
for (let y = 0; y < H; y++) for (let x = 0; x < W; x++) {
  ...
  out[d] = mapped[s]; out[d + 1] = mapped[s + 1]; out[d + 2] = mapped[s + 2]; out[d + 3] = 255;
}
...
console.log(`[done] wrote ${OUT}: ${nz}/${W * H} non-black pixels (${(100 * nz / (W * H)).toFixed(1)}%)`);
console.log(nz > 1000 ? 'RESULT: STAGE RENDERS' : 'RESULT: STAGE BLANK — gate/draw failing');
