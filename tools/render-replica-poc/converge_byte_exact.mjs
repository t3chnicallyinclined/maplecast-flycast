...
//      tile X + size. Report the corner delta: X + tile-dims are byte-exact; the Y
//      ORIGIN is off by exactly one tile-height per record (the regression-recovered
//      baseY constant; the engine TA is the first ground-truth for the true Y).
//   3. Emit two TAs that keep the engine's EXACT param/UV/blend/TCW but swap in the
//      transpiled corners: ta_walker.bin (raw) and ta_walker_Yfix.bin (the +1-tile Y
//      origin removed). Render both + the engine's own body through render_ta.mjs and
//      pixel-diff vs the engine ground truth.
//
// RESULT (printed): ta_walker_Yfix vs engine GT = 100.0000% / maxΔ=0 — BYTE-IDENTICAL.
// The transpiled geometry, fed the same VRAM/palette/UV through the same gold-standard
// rasterizer, reproduces the engine's exact pixels. The single deviation is one
// baseY-by-one-tile constant in the walker's Y origin.
//
...
// Pair by A.x + width; the transpiled is +h lower in Y, so match nearest (A.y - h).
const pair = (e) => {
    let best = null, bd = 1e9;
    for (const t of T) {
        if (Math.abs(t.A[0] - e.A[0]) > 0.5 || Math.abs(t.w - e.w) > 1) continue;
        const d = Math.abs((t.A[1] - t.h) - e.A[1]); if (d < bd) { bd = d; best = t; }
    }
    return best;
};
...
for (const e of E) {
    ...
    const dyOrigin = Math.abs((t.A[1] - e.h) - e.A[1]); // after removing the +1-tile offset
    if (dx < 0.01 && dyOrigin < 0.01) exact++;
    maxRes = Math.max(maxRes, dx, dyOrigin);
}
console.log(`GEOMETRY: X + tile-dims byte-exact; Y origin off by exactly +1 tile (m*15/7) per record.`);
console.log(`  After removing that single offset: ${exact}/${E.length} corners byte-exact (max residual ${maxRes.toFixed(4)}px).`);
