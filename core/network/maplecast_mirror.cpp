/*
	MapleCast Mirror — stream TA command buffers + memory diffs over WebSocket.

	Server: each frame, delta-encodes TA commands + VRAM/PVR diffs into a local
	        buffer, then sends via WebSocket (maplecast_stream::broadcastBinary).

	Optimizations:
	  - TA delta: double-buffered pointers, zero-copy swap (no prevTA.assign())
	  - VRAM diff: uses flycast's memwatch page-fault tracking instead of scanning
	    all 2048 pages. Only pages the game actually wrote to get diffed.
	  - PVR regs: still scanned (only 8 pages, ~32KB, negligible)
*/
#include "types.h"
...
// Double-buffered TA pointers for zero-copy delta
// Instead of copying 140KB every frame with prevTA.assign(),
// we just swap which buffer is "current" vs "previous"
static uint8_t* _taBuf[2] = { nullptr, nullptr };
static uint32_t _taBufSize[2] = { 0, 0 };
static int _taCur = 0;  // index of buffer to write INTO this frame
static bool _taHasPrev = false;

// PVR register shadow — small enough to just memcmp (8 pages, 32KB)
...
void initServer()
{
	...
	// Enable VRAM page-fault tracking (same system GGPO uses)
	// This protects VRAM pages so the OS tells us exactly which ones the game wrote to
	memwatch::mirrorActive = true;
	memwatch::vramWatcher.protect();

	printf("[MIRROR] === SERVER MODE === WebSocket-only, memwatch VRAM tracking enabled\n");
}

void initClient()
{
	_isClient = true;
	printf("[MIRROR] === CLIENT MODE ===\n");
}
...
// ==================== SERVER: build delta frame, send over WebSocket ====================
...
void serverPublish(TA_context* ctx)
{
	...
	// === TA command buffer — fused copy + diff + checksum in one pass ===
	// One read of the TA buffer does three things:
	//   1. Copies into our double buffer (for next frame's diff)
	//   2. Delta-encodes against previous frame (for network send)
	//   3. Computes XOR checksum (for client verification)
	...
	if (canDelta)
	{
		...
		// Fused pass: copy taData→curBuf, checksum, and find changed runs
		uint32_t i = 0;
		while (i < taSize)
		{
			// Skip unchanged bytes — copy + checksum + compare in one loop
			while (i < commonSize) {
				uint32_t word;
				memcpy(&word, taData + i, 4);
				taChecksum ^= word;
				memcpy(curBuf + i, taData + i, 4);  // copy
				if (*(uint32_t*)(taData + i) != *(uint32_t*)(prevData + i)) break;
				i += 4;
			}
			// Handle tail bytes past commonSize
			if (i >= commonSize) {
				// Copy + checksum remaining bytes that are past previous buffer
				while (i + 3 < taSize) {
					uint32_t word;
					memcpy(&word, taData + i, 4);
					taChecksum ^= word;
			...
			// Include trailing bytes to avoid tiny gaps
			if (i < taSize) {
	...
	{
		// Keyframe: copy + checksum in one pass, send full TA
		...
		for (uint32_t i = aligned; i < taSize; i++)
			curBuf[i] = taData[i];
		if (taSize > 0) { memcpy(dst, taData, taSize); dst += taSize; }
	}

	// Swap double buffer for next frame
	_taBufSize[cur] = taSize;
	_taCur = prev;
	_taHasPrev = true;

	// Checksum already computed during the fused pass
	...
	// VRAM: use memwatch page-fault tracking — only pages the game wrote to
	// getPages() swaps out the dirty set and returns it. Zero scanning.
	memwatch::PageMap vramDirty;
	memwatch::vramWatcher.getPages(vramDirty);

	for (const auto& pair : vramDirty) {
		uint32_t pageOff = pair.first;  // offset within VRAM
	...
	// Re-protect VRAM for next frame's tracking
	memwatch::vramWatcher.protect();

	// PVR regs: only 8 pages (32KB), just memcmp — negligible cost
	{
