...
// v2: per-draw state via storage buffer indexed by polyId vertex attribute
...
// packed: low 8 bits = debug mode, bit 8 = Gouraud, bit 9 = isTrans, bit 10+ = discard flags
...
@fragment fn fs_main(in: FIn) -> FOut {
    ...
    if (noDiscard == 0u) {
        if (discTransOnly == 1u) { if (isTrans == 1u && c.a < 0.004) { discard; } }
