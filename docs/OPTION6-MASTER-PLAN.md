# Option 6 — Master Plan (post-Phase-2 pivot) [ARCHIVED RESEARCH]

> **🪦 OUTCOME (2026-05-07): Approach abandoned. Branch retired.**
>
> **What happened:**
> - Phase 1 (TexRefs bundling): 0% hit rate, 5.7 GB cache from 10 min
> - Phase 2 (CSV-decomposed VisualKey): 0% hit rate, over-discriminated keys (key_frame ticked every frame)
> - **Pivot A (TA-buffer dedup ring with REUSE opcodes — recommended below): 0% hit rate** in MVC2. Per-frame timer + HUD + animation tick make every TA buffer unique even on stationary frames. Hashes never collide.
>
> **Where the saving actually came from:**
> Geographic optimization (an EWR community node closer to the player) cut RTT from ~21ms to ~5ms — a **bigger latency win than any cache-based bandwidth scheme would have produced**, with zero new architecture. See [OPTIMIZATION-PLAN.md](OPTIMIZATION-PLAN.md). The bandwidth question (4 Mbps in-match) was never the bottleneck a streaming user cared about — the latency question is.
>
> **What was kept from this branch (cherry-picked to master):**
> - `c9a8947d4` SOCKET typedef on non-Windows (build portability)
> - `7cffb5979` skip gui_displayMirrorDebug() in headless (crash fix)
> - `2966b016a` MAPLECAST_INPUT_TARGET_PORT env override (generic env knob)
>
> **What was deleted:**
> - `core/network/maplecast_visual_cache.{cpp,h}`, `maplecast_scanner.{cpp,h}`, `maplecast_lookup_test.{cpp,h}`
> - `deploy/scripts/deploy-lookup.sh`, `deploy/systemd/maplecast-lookup.{env,service}`
> - TA-buffer dedup ring code in `maplecast_mirror.cpp` (Pivot A from this doc)
> - Production sandbox: stopped/disabled `maplecast-lookup.service`, removed `/usr/local/bin/flycast-lookup`, cleaned ufw rules for 7250/7253/7150
>
> **Lessons preserved for future cache work:**
> - The renderer wants **draw commands**, not game state — the right key shape (`(TCW,TSP)`) was already in `web/webgpu/texture-manager.mjs`
> - Per-frame timer/animation/HUD ticks defeat ANY full-frame hashing scheme
> - Cache invalidation has to be VRAM-page-driven, not state-driven
> - LiveRender (HKUST ACM MM '14) is the relevant academic precedent: hash draw-call parameters, never surrounding state. 52-73% bandwidth, ~thousands of cache entries
>
> **If we ever revisit:** the right level for sprite/draw dedup is per-draw-call (Pivot B in this doc, ~3-5 days) operating on TA-stream parameters, not whole frames. But the latency-first focus (rollback prediction, geographic distribution) wins more user value per engineering hour. See [OPTION6-INSANE-IDEAS.md](OPTION6-INSANE-IDEAS.md) for the wild idea pile that came out of this exercise.
